...
public class Program {
	...
	private static HtmlView<Task> taskDetailsView() {
		...
		taskView.body().heading(1, "Task Details").hr().div().text("Id: ")
				.text(binderGetId()).br().text("Title: ")
				.text(binderGetTitle()).br().text("Description: ")
				.text(binderGetDescription()).br().text("Priority: ")
	...
	private static void testTasks() throws IOException {
		...
		Task t2 = new Task(
				"Concluir AOM compiler v2",
				"Description of the task",
				Priority.Normal, Status.Unstarted);
		Task t3 = new Task("Jantar do dia do pai",
				"Marcar mesa num restaurante interessante!", Priority.High,
				Status.Progress);
		try {
		...
		try {
			PrintStream out = new PrintStream(new FileOutputStream(
					"TaskList.html"));
			taskListView().setPrintStream(out).write(1,
